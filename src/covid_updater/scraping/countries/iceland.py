...
class IcelandScraper(IncrementalScraper):
    ...
    def load_data(self):
        ...
        df = (
            pd.DataFrame.from_records(data[1:], columns=["region", "people_vaccinated", "people_fully_vaccinated"])
        )
    ...
    def _process(self, df):
        ...
        df = df.assign(
            date=(datetime.now(pytz.timezone("Atlantic/Reykjavik")).date() - timedelta(days=1)).strftime("%Y-%m-%d")
        )
        return df
    ...
    def _postprocess(self, df):
        ...
        df.loc[df.region=="Nordurland", "region_iso"] = "IS-6,IS-7"  # Also accounts for IS-5 ! TODO
        return df
