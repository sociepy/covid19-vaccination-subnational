...
class SpainScraper(Scraper):
    ...
    def load_data(self):
        # Load
        df = pd.read_csv(self.data_url, dtype={
    ...
    def _process(self, df):
        ...
        df.loc[:, "total_vaccinations"] = df.loc[:, "total_vaccinations"].apply(self.remove_delimiter)
        df.loc[:, "people_fully_vaccinated"] = df.loc[:, "people_fully_vaccinated"].apply(self.remove_delimiter)
